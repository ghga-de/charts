...
spec:
  ...
  authorization:
    ...
    {{- $kafkaUser = (merge $kafkaUser (dict "resource" (dict "name" ($.Values.topicPrefix | empty | ternary (cat  $topicValue.topic.value "-" $.Values.serviceNameConsumer "-" $topicValue.topic.value) (cat $.Values.topicPrefix "-" $.Values.serviceNameConsumer) | nospace)))) -}}
    {{- else if and (eq $topicKey "deadLetterQueueRetry") $.Values.serviceName }}
    {{- $kafkaUser = (merge $kafkaUser (dict "resource" (dict "name" ($.Values.topicPrefix | empty | ternary (cat  $topicValue.topic.value "-" $.Values.serviceName "-" $topicValue.topic.value ) (cat $.Values.topicPrefix "-" $.Values.serviceName) | nospace)))) -}}
    {{- else }}
